#!/bin/bash
#
# run_nccl_test.sh — Run NCCL tests on Slurm via Pyxis/enroot containers
#
# Usage:
#   ./run_nccl_test.sh                          # 1 node, all_reduce, default
#   ./run_nccl_test.sh -n 2 -t all_reduce       # 2 nodes, all_reduce
#   ./run_nccl_test.sh -n 4 -t all_gather -p nvcheck
#   ./run_nccl_test.sh -n 1 -t sendrecv --dry-run
#
...
ITERS=20
WARMUP=5
CONTAINER_IMAGE="/mnt/vast/dgxc-benchmarking-auto/containers/nccl-tests-cuda12.9.sqsh"
EXTRA_ARGS=""
DRY_RUN=0
JOB_NAME=""
OUTPUT_DIR=""
...
usage() {
    cat <<'USAGE'
Usage: run_nccl_test.sh [OPTIONS]
...
Options:
  ...
  -c, --container PATH     Container image path (sqsh or docker URI)
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -c|--container)   CONTAINER_IMAGE="$2"; shift 2 ;;
